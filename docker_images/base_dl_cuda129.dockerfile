...
# Install required packages (including tmux), run pip installs, then purge build deps to reduce image size
RUN apt-get update && apt-get -y upgrade && \
    ...
    # Remove build tools which are no longer needed to keep image small
    apt-get purge -y --auto-remove build-essential && \
    apt-get autoremove -y && apt-get clean && \
    rm -rf /var/lib/apt/lists/* /root/.cache/pip /tmp/* /var/tmp/* /var/cache/apt/* /usr/share/doc/* /usr/share/man/*
